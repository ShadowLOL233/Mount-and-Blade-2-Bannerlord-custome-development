...
**状态**：**进行中 v0.3**（2026-09-20）· 本文件**着重兵种命名**，背景设定仅作命名依据的骨架。**完成进度见下方 Status 节。**
**世界观基座**：骑马与砍杀 II 卡拉迪亚（Calradia）。

---

## 完成进度 / 待办（Status · 2026-09-20）
...
- ⬜ **外籍军团（Foederati）成树**：诺德 / 斯特吉亚 / 瓦兰迪亚 / 巴旦尼亚——目前 §五 仅扁平命名池，未组织成分支小树
- ⬜ **蛮族 2H 冲击步兵是否并入**（falx / 罗斯斧 / 法兰克长柄，填 core 缺的 2H 打击手）——待定
- ⬜ **装备与技能数值配置**——用户明确暂缓（当前只做命名 + 分支）
- ⬜ **精锐线融合/近卫分支的装备细化**（D/E/F 已有名，装备待定）
- ⬜ **清理 / 对账 §三～§七 旧通用命名池**——部分已被上方两个 ★ 节取代，§五 外籍名仍作参考
- ⬜ **开放命名问题（见 §七）**：帝国形容词 `Thaddian / Thaddean / Sadian`；是否连 vanilla 旧帝国兵一并改名
...
1. **希腊 vs 拉丁配比**：目前核心偏**希腊**（更"拜占庭/东罗马"）。要不要某些线改用更硬的**拉丁**（Legionarius/Sagittarius/Eques）以呼应"承接罗马"？
2. **是否重命名 vanilla 帝国兵**：Retinues 是给你自定义兵起名——旧帝国 vanilla 兵是否也想统一改成本套（还是仅新帝国用）？
3. **帝国形容词**：`Thaddian` / `Thaddean` / 直接 `Sadian`？影响所有"帝国御用"前缀写法。
4. **tier 深度**：你的 Retinues `MaxTroopTier=10`——要不要把顶级近卫铺到 T7–T10（例如 Basilikos 系列专属 T8+）?
5. **外籍军团是否分"雇佣阶段 vs 归化阶段"两套名**（呼应设定 3「最终融入帝国」）——雇佣期用纯外族名，归化后加帝国前缀？

---

*草案由 Claude Code 协助生成 2026-09-20。命名以拜占庭军事术语为骨架，可按上面 5 个分叉调整。*
